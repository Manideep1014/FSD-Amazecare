...
namespace AmazeCareProject.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public IActionResult roleBasedAuth([FromBody] User user)
        {
            ...
            if (user != null)
            {
                var dbPatient = _dbContext.Patient.FirstOrDefault(p => (p.UserName == user.UserName) && (p.Password == user.Password));
                if (dbPatient != null)
                {
                    return Token("Patient", user, dbPatient.PatientId);
                }

                var dbDoctor = _dbContext.Doctors.FirstOrDefault(p => (p.UserName == user.UserName) && (p.Password == user.Password));
                if (dbDoctor != null)
                {
